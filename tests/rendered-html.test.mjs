...
test("keeps interaction scoped and accessibility preferences explicit", async () => {
  ...
  assert.match(nativePortfolioVideo, /controls=\{showNativeControls\}/);
  assert.match(nativePortfolioVideo, /video\.controls = false/);
  assert.doesNotMatch(nativePortfolioVideo, /video\.controls = true/);
  ...
  assert.match(nativePortfolioVideo, /onPlaying=\{\(event\) =>/);
  assert.match(nativePortfolioVideo, /requestAnimationFrame\(\(\) =>/);
  assert.match(nativePortfolioVideo, /setShowNativeControls\(true\)/);
