...
test("keeps interaction scoped and accessibility preferences explicit", async () => {
  ...
  assert.match(booking, /data-state=\{open \? "open" : "closed"\}/);
  ...
  assert.match(css, /\.booking-overlay\s*\{[^}]+--booking-overlay-x:\s*clamp\(2rem, 5vw, 6rem\)[^}]+--booking-overlay-y:\s*clamp\(1\.5rem, 6vh, 4\.5rem\)[^}]+padding:\s*var\(--booking-overlay-y\) var\(--booking-overlay-x\)/s);
  ...
  assert.match(css, /\.booking-calendar\s*\{[^}]+width:\s*min\(100% - 2rem, 22\.5rem\)[^}]+align-self:\s*center[^}]+justify-self:\s*center/s);
  assert.match(css, /\.booking-calendar iframe\s*\{[^}]+width:\s*100%[^}]+height:\s*min\(40\.5rem, calc\(100dvh - \(2 \* var\(--booking-overlay-y\)\) - 7rem\)\)[^}]+min-height:\s*35rem/s);
  assert.match(css, /\.booking-wordmark\s*\{[^}]+font-size:\s*clamp\(6\.25rem, 14vw, 15\.5rem\)[^}]+pointer-events:\s*none/s);
  ...
  assert.match(css, /@media \(max-width: 50rem\)[\s\S]+\.booking-calendar iframe\s*\{[^}]+height:\s*43rem[^}]+min-height:\s*43rem/s);
