...
test("keeps interaction scoped and accessibility preferences explicit", async () => {
  const [page, pageFrame, siteFooter, layout, header, booking, availabilityRoute, contactForm, contactSubmit, portfolio, shortFormViewer, workPage, workShortFormCard, caseStudyMedia, mediaPlayback, data, theme, timeline, css, worker, viteConfig, swanLogo, favicon] = await Promise.all([
  ...
  assert.match(header, /href === "\/#services" && window\.location\.pathname === "\/"/);
  assert.match(header, /pendingServiceNavigationRef\.current = \{ hash: "#services", reducedMotion \}/);
  assert.match(header, /serviceScrollFrameRef\.current = window\.requestAnimationFrame\(\(\) => \{\s*serviceScrollFrameRef\.current = window\.requestAnimationFrame/s);
  assert.match(header, /window\.getComputedStyle\(destination\)\.scrollMarginTop/);
  assert.match(header, /window\.scrollTo\(\{ top: Math\.max\(0, targetTop\), behavior: pendingNavigation\.reducedMotion \? "auto" : "smooth" \}\)/);
  ...
  assert.match(header, /New Delhi · <time>\{delhiTime\}<\/time>/);
  assert.match(header, /<span>View availability<\/span>/);
  ...
  assert.match(css, /\.mobile-availability\s*\{[^}]+min-height:\s*2\.5rem[^}]+border:\s*1px solid var\(--line\)[^}]+border-radius:\s*99rem/s);
